...
// Kullanıcı girişlerini kontrol etme
...
$stmt = $conn->prepare("SELECT adi, soyadi, numara, okulagiristarihi FROM ogrenciler WHERE DATE(okulagiristarihi) = ? LIMIT ?, ?");
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-end'>" . $row["okulagiristarihi"] . "</td>";
        echo "</tr>";
    }
} else {
    echo "<style>#nextEnterBtn { display: none; }</style>";
    echo "<tr><td colspan='2'>Bugün okula giriş yapan öğrenci bulunamadı.</td></tr>";
}
